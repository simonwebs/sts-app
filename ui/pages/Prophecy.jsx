...
const Prophecy = () => {
  ...
  const { posts, isLoading } = useTracker(() => {
    const handle = Meteor.subscribe('posts.byCategory', 'prophecy');

    if (!handle.ready()) {
      return { posts: [], isLoading: true };
    }

    const prophecyPosts = PostsCollection.find({ category: 'prophecy' }).fetch();
    const authorIds = prophecyPosts.map(post => post.authorId);
    const authors = Meteor.users.find({ _id: { $in: authorIds } }).fetch();

    const postsWithAuthor = prophecyPosts.map(post => ({
      ...post,
   ...
   <div className="bg-white dark:bg-gray-700 py-24 sm:py-32">
            <div className="mx-auto max-w-7xl px-6 lg:px-8">
                <div className="mx-auto max-w-2xl text-center">
                    <h2 className="text-3xl font-bold tracking-tight text-gray-900 dark:text-gray-100 sm:text-4xl">From the prophecy department</h2>
