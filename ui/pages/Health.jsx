  ...
  if (authorImage) {
    return (
      <Image
        ...
        dpr="auto"
        responsive
        className="h-8 w-8 sm:h-10 sm:w-10 rounded-full ring-4 ring-white"
        alt="Author profile"
      />
    );
  }
...
const Health = () => {
  ...
  const { posts, isLoading } = useTracker(() => {
    ...
    const healthPosts = PostsCollection.find({ category: 'health' }).fetch();
    const authorIds = healthPosts.map(post => post.authorId);
    const authors = Meteor.users.find({ _id: { $in: authorIds } }).fetch();

    const postsWithAuthor = healthPosts.map(post => ({
      ...post,
   ...
   <div className="bg-white dark:bg-gray-700 py-24 sm:py-32">
            <div className="mx-auto max-w-7xl px-6 lg:px-8">
                <div className="mx-auto max-w-2xl text-center">
                    <h2 className="text-3xl font-bold tracking-tight text-gray-900 dark:text-gray-100 sm:text-4xl">From the health department</h2>
